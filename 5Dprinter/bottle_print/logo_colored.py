out = open("_logo_colored.gcode","w")
...
for z in range(0,3):
    ...
    if z == 0:
        zShift = 0
        for i in reversed(range(1,55)):
            zShift += 0.016
            out.write("G1 Z"+str(0.2+zShift)+" F"+str(speed)+"\n")
            out.write("G1 X%f Y%f E%f F%f\n" % (30,-10*(i+1),amount*10,speed))
        out.write("G1 X5 Z5.2 F"+str(speed)+"\n")
        out.write("G1 X%f Y%f E%f F%f Z%f\n" % (0,-60,0,speed,(0.3*z)+0.2))
...
for i in range(1,100):
    zShift += 0.016
    out.write("G1 Z"+str(0.2+zShift)+" F"+str(speed)+"\n")
    out.write("G1 X%f Y%f E%f F%f\n" % (-11,-10*(i+1),amount*10,speed))
out.write("G1 Z"+str(2.2+zShift)+" F"+str(speed)+"\n")
out.write("G1 X%f Y%f E%f F%f\n" % (39,-10,amount*0,speed))
zShift = 0
for i in range(1,100):
    zShift += 0.016
    out.write("G1 Z"+str(0.2+zShift)+" F"+str(speed)+"\n")
    out.write("G1 X%f Y%f E%f F%f\n" % (38,-10*(i+1),amount*10,speed))

out.write("G1 Z"+str(3.4)+" F"+str(speed)+"\n")
out.write("G1 X%f Y%f E%f F%f\n" % (-10.5,-90,amount*0,speed))
out.write("G1 Z"+str(1.4)+" F"+str(speed)+"\n")
out.write("M104 S190 ; set extruder temp\n")
for i in range(-90,-19):
    out.write("G1 X%f Y%f E%f F%f\n" % (-12,i,amount*70,speed))
    out.write("G1 X%f Y%f E%f F%f\n" % (40,i,amount*70,speed))
out.write("G1 Z"+str(1.8)+" F"+str(speed)+"\n")
for i in reversed(range(-12,41)):
    out.write("G1 X%f Y%f E%f F%f\n" % (i,-90,amount*50,speed))
    out.write("G1 X%f Y%f E%f F%f\n" % (i,-20,amount*50,speed))
out.write("G1 Z"+str(2.1)+" F"+str(speed)+"\n")
for i in range(-90,-19):
    out.write("G1 X%f Y%f E%f F%f\n" % (-12,i,amount*70,speed))
    out.write("G1 X%f Y%f E%f F%f\n" % (40,i,amount*70,speed))
out.write("G1 Z"+str(2.3)+" F"+str(speed)+"\n")
for i in reversed(range(-12,41)):
    out.write("G1 X%f Y%f E%f F%f\n" % (i,-90,amount*50,speed))
    out.write("G1 X%f Y%f E%f F%f\n" % (i,-20,amount*50,speed))
...
out.write("G1 Z"+str(4.4)+" F"+str(speed)+"\n")
...
with open("logo_upper.gcode") as includedFile:
    ...
    for line in includedFile.read().split("\n"):
        ...
        while counter2 < len(splitedLine):
            chunk = splitedLine[counter2]
            if chunk.startswith("Z"):
                chunk = "Z%f"%(float(chunk[1:])+2.75)
            if chunk.startswith("X"):
                chunk = "X%f"%(float(chunk[1:])-20.0)
            if chunk.startswith("Y"):
                chunk = "Y%f"%(float(chunk[1:])-89.5)
            if chunk.startswith("F"):
                chunk = "F1200"
            if chunk.startswith("E"):
                newChunk = "E%f"%((float(chunk[1:])-e)*1.2)
